 ...
 * a real number of distinct shell-injection, Net::HTTP-in-loop, and ungrouped-Gemfile-gem findings flagged -- never on install, never on
...
const KEY_SEEN_FINDINGS = 'rubyCompanion.review.seenFindings';
const KEY_ANSWERED = 'rubyCompanion.review.answered';
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Ruby Companion: this has flagged ${HITS_BEFORE_PROMPT} real issues in your code -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
